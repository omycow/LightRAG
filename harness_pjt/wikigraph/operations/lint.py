...
Most checks are pure graph algorithms (0 LLM tokens). The only automatic
fix is removing stale EVOLVE-inferred edges — structural edges
(``wikigraph_structural``) are never auto-removed here, since they encode
real corpus facts (document X really does contain entity Y) rather than
inferences, so "never accessed" isn't a reason to delete them.
"""
...
async def lint_node(
    ...
    for node_id in all_labels:
        ...
        if degree == 0:
            findings.append(LintFinding(
                ...
                severity="warning",
                entity_name=node_id,
                details="Entity has no connections (degree=0)",
                ...
                if 1 < len(entities) <= 20:
                    for i in range(len(entities)):
                        for j in range(i + 1, len(entities)):
                            candidate_pairs.add(tuple(sorted((entities[i], entities[j]))))
    ...
    # AUTO-FIX: remove stale EVOLVE-inferred edges that are never accessed.
    # Keyed off `keywords`, not `source_id` — see the tagging note in
    # evolve.py for why `source_id` can't be trusted for injected edges.
    fixed = 0
    try:
        all_edges = []
        for node_id in all_labels:
        ...
        for src, tgt in all_edges:
            ...
            if "wikigraph_evolve" not in keywords:
                continue  # only auto-clean EVOLVE-inferred edges, never structural ones
            weight = float(edge.get("weight", 1.0))
            if weight > 0.5:
                continue
            src_access = entity_meta.get(src, {}).get("access_count", 0)
            tgt_access = entity_meta.get(tgt, {}).get("access_count", 0)
