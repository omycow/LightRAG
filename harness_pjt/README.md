...
### 전략 6: Artifact Ingestion (산출물 재삽입)

**문제**: 에이전트가 쿼리 답변, 요약, 분석 리포트 등 지식 산출물을 생성하지만, 이것이 그래프에 반영되지 않아 다음 쿼리에서 활용할 수 없습니다. LLM Wiki에서 좋은 답변이 위키 페이지로 편입되는 흐름이 빠져있습니다.

**동작**:
1. `agent.ingest_artifact(text, label)`로 산출물 텍스트를 전달합니다
2. 내부적으로 `rag.ainsert()`를 호출하여 일반 문서와 동일한 파이프라인(청킹 → LLM 추출 → 임베딩 → 그래프)을 거칩니다
3. 산출물에서 추출된 엔티티/릴레이션이 그래프에 추가됩니다

**효과**: 에이전트의 분석 결과가 그래프에 축적됩니다. 예를 들어 "LightRAG 아키텍처 요약"이라는 답변을 재삽입하면, 그 요약에서 추출된 엔티티/관계가 다음 쿼리에서 검색됩니다.
...
### 전략 7: Document Sync (원본 문서 변경 동기화)

**문제**: 원본 문서가 수정되거나 새 문서가 추가되었지만, 그래프는 이전 버전을 반영하고 있습니다. 수동으로 다시 삽입하지 않으면 그래프가 최신 상태를 유지하지 못합니다.

**동작**:
1. `agent.watch(directory)`가 디렉토리의 모든 파일에 대해 content hash를 계산합니다
2. 이전 해시와 비교하여 **변경된 파일만** 탐지합니다
3. 변경된 파일을 `rag.ainsert()`로 재삽입합니다 (기존 엔티티는 description 병합)
4. 삭제된 파일은 전략 4(Source Verification)가 다음 EVOLVE 사이클에서 근거 상실 엣지를 제거합니다

**효과**: 감시 디렉토리에 파일을 추가/수정하기만 하면 그래프가 자동으로 따라갑니다.

```python
result = await agent.watch("./docs/")
# → "3 file(s) changed" → 자동 ingest → 그래프 업데이트
```
...
| 전략 | 소스 | 동작 | 그래프 변경 |
|---|---|---|---|
| 1. Co-retrieval | 쿼리 로그 | 공동 검색 패턴 → 새 엣지 | **추가** |
| 2. Gap Filling | 원본 청크 | 추출 누락 재추출 | **추가** (노드+엣지) |
| 3. Shortcut | 쿼리 로그 | 다중 홉 단축 | **추가** |
| 4. Source Verification | text_chunks | 근거 청크 소멸 확인 | **제거** |
| 5. Contradiction | 그래프 노드 | 모순 description 통합 | **수정** |
| 6. Artifact Ingestion | 에이전트 산출물 | 답변/요약 → 그래프 삽입 | **추가** |
| 7. Document Sync | 파일 시스템 | 변경 파일 감지 → 재삽입 | **추가/갱신** |
