...
- **EVALUATE**: 리트리브 결과마다 **quality** 점수(0.0~1.0)를 매깁니다. LLM 호출 없이 순수 규칙 기반입니다.
...
리트리브 결과가 얼마나 "쓸만했는지"를 나타내는 지표입니다. 그래프 개선 실행 여부를 막는 게이트는 아니고 — tier 1은 매 쿼리 백그라운드로 도니 항상 실행됩니다 — 로그 분석·모니터링용으로 남습니다.
...
자세한 계산 규칙은 [ARCHITECTURE.md의 quality](ARCHITECTURE.md#quality-쿼리-품질-점수--2-로깅-및-로그분석이-산출-0-llm-호출)를 참고하세요.
...
원본 문서에서 추출된 관계와 그래프 개선이 추가한 관계를 구분하기 위해, 그래프 개선이 새로 만드는 모든 엣지에는 **weight**(엣지 가중치)가 낮게 매겨집니다. 검색 시 `(rank, weight)` 순으로 정렬되므로, weight가 낮을수록 원본 추출 관계보다 후순위로 밀립니다.

| 출처 | weight |
|---|---|
| 원본 문서에서 LLM 추출 | **1.0** (여러 문서에서 재추출되면 누적) |
| 로그 기반 추론 (co-retrieval/gap-fill/shortcut) | **0.5** |
| 코퍼스 메타데이터 기반 구조 마이닝 | **0.3** |

구조 마이닝 엣지가 더 낮은 이유: 항상 참이지만(문서가 실제로 그 엔티티를 포함), 의미적으로는 로그 기반 추론보다 느슨한 신호이기 때문입니다. 자세한 내용은 [ARCHITECTURE.md의 weight](ARCHITECTURE.md#weight-엣지-가중치--lightrag-기본-제공-3-그래프-개선이-부여)를 참고하세요.
...
| Evolving LightRAG (그래프 개선 적용) | 64% ~ 72% (변동) |

그래프 개선이 항상 vanilla보다 낫진 않습니다 — 구조 마이닝 엣지가 기존 지식그래프에 노이즈로 섞이는 것으로 보이며, 자세한 원인 분석과 대응 계획(구조 그래프 분리)은 [ARCHITECTURE.md의 벤치마크](ARCHITECTURE.md#벤치마크)를 참고하세요.

---

## 한계와 열린 질문

현재 구현은 아키텍처 제안과 프로토타입 단계입니다. 위 벤치마크에서 보듯 그래프 개선이 검색 품질을 항상 개선하는 건 아니고, 구조 관계가 멀티홉 질의에 실질적으로 도움이 되는지도 정량적으로 검증되지 않았습니다. 자세한 위험 분석과 TODO는 [ARCHITECTURE.md의 한계](ARCHITECTURE.md#한계)를 참고하세요.
