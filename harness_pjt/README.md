...
### 전략 2: Gap Filling (지식 갭 채우기)

**문제**: 특정 쿼리에 대해 그래프에서 관련 엔티티/관계가 전혀 검색되지 않습니다 (quality < 0.3). 원본 문서에 해당 지식이 없거나, 추출 과정에서 누락된 것입니다.

**동작**:
1. 쿼리 로그에서 quality < 0.3인 실패 쿼리를 수집합니다
2. LLM에게 해당 쿼리에 답하기 위해 필요한 엔티티와 관계를 제안하도록 요청합니다
3. 응답을 파싱하여 새 엔티티 + 릴레이션을 `ainsert_custom_kg()`로 주입합니다

**효과**: 반복적으로 실패하는 쿼리 영역에 **지식을 보충**합니다. 다음 동일 쿼리 시 결과가 개선됩니다. 이 전략만이 **새 엔티티 노드**를 추가할 수 있습니다 (나머지 전략은 기존 노드 간 엣지만 추가).
