...
_SKEL_TYPES_C = [("test", ("테스트", "test", "검증", "validation")),
                 ("spec", ("스펙", "spec", "specification", "규격"))]
_COMPLEMENT_FACETS = {"test": ["스펙 spec 규격", "이슈 issue 문제 보고"],
                      "spec": ["검증 테스트 test"]}
_TYPE_WORDS_C = {h for _, hints in _SKEL_TYPES_C for h in hints} | {"이슈", "issue"}


def complementary_facet(query: str) -> list[dict]:
    """v5.34: rule-generated companion facets — the INSTANT (non-learned) part of
    the facet family. Joins fusion immediately; no facet_link accumulation (the
    drift source measured in ALL/G runs). Korean 2-char tokens included."""
    import re as _re
    q = query.lower()
    dtype = next((n for n, hints in _SKEL_TYPES_C if any(h in q for h in hints)), None)
    if dtype not in _COMPLEMENT_FACETS:
        return []
    from harness_pjt.structrag.quality import _KO_PARTICLE
    toks = _re.findall(r"[A-Za-z0-9][A-Za-z0-9_\-]{2,}|[가-힣]{2,}", query)
    stop = {"있고", "있어", "있나요", "있는지", "대한", "어떤", "무엇", "관련", "그리고"}
    content = []
    for t in toks:
        t = _KO_PARTICLE.sub("", t.lower()) if _re.search(r"[가-힣]", t) else t.lower()
        if len(t) >= 2 and t not in _TYPE_WORDS_C and t not in stop and t not in content:
            content.append(t)
    if len(content) < 2:
        return []
    return [{"q": " ".join(content[:8]) + " " + suf, "intent": "cross_doc",
             "mode": "hybrid", "_complement": True} for suf in _COMPLEMENT_FACETS[dtype]]
...
class QueryAnalyzer:
    ...
    def _finalize(self, plan: QueryPlan) -> QueryPlan:
        # v5.34: instant companion facets (rule part of the facet family only)
        ...
        for sub in plan.subqueries:
            if sub.get("_complement"):
                continue  # synthetic facet stays on hybrid (no kw-cache misses)
