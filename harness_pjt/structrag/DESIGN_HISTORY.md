...
### 2026-07-15 — v5.33-ALL 판정 (조기 종료) 및 v5.34
- ALL: iter1 71.1 (균일 레이턴시 역대 최고 출발 — 상보 facet 융합 참여 + 당회 교차증거의 **룰 효과**)
  → 71.1→67.8→66.7→62.2 4연속 하락 (facet_link 학습 드리프트가 L1 위에서도 재현). 유저 판단으로
  iter5 생략, **기각**. 분해 결론: facet의 룰 부품 = 즉시 유효 / 학습 부품 = 순손실.
- v5.34 = v5.20 + 룰 부품만: ① 상보 facet 상시 융합 참여 (한글 수정 추출기, hybrid 고정)
